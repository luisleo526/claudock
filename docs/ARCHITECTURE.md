...
| `ShellIntegration` | Opt in to a namespaced CLI function and a marked `.zshrc` loader; back up edits and preserve unrelated user content. |
...
`SessionAnalytics` reads main JSONL sessions, direct subagent logs, and recognized workflow logs under `<project>/<sessionUUID>/subagents/workflows/wf_*/` for a selected 7- or 30-day window. It resolves the explicitly selected config and `projects` roots, including symlinked roots, while rejecting symlinked descendants/files. Multiple profiles that resolve to the same history root share one scan and contribute to the `shared-history` bucket, displayed as **Shared history**. CLI logs do not provide a reliable account identity, so shared-root totals remain unattributed to individual accounts. Per-profile totals require isolated history folders.
...
Shell integration is a separate, optional mutation. It writes `~/.config/claudock/init.zsh` and a marked loader block in `.zshrc`, taking an exact backup before modifying an existing shell file. The init file exposes only the `claudock` function bound to the installed app's bundled executable. It does not shadow `claude` or any `claude-NAME` command. Each CLI call reads the current registry, so profile mutations need no shell rewrite or reload. Enabling/disabling the integration itself requires a new shell to load/unload the function. Re-enable integration after moving the app to update the executable path.
